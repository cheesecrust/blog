...
### state
state는 component 에서 들고 있는 값 입니다.
### props 
props는 상위 컴포넌트에서 하위로 인자를 통해서 내려주는 값

## state 

- 무엇을 state 로 설정해야 하는가?
	state는 app이 기억해야하는 최소한의 변경 가능한 data 의 집합으로 표현됩니다.
	
	state 로 설정하지 않는 데이터 특징
	1. 시간이 지나도 변하지 않는다.
	2. 부모 class 에서 props 를 통해 전달합니다.
	3. 컴포넌트의 기존 상태나 속성을 기반으로 연산할 수 있습니다.	   
...
- state 의 변경은 어떻게 관리해야 하는가?
	state 의 변경은 react 의 hook 인 useState 를 통해 set 함수를 통해서 state 의 값을 변경합니다.
	이때의 set callback 함수를 인자로 같이 넘겨서 하위 컴포넌트에서 값을 설정할 수 있도록 합니다.